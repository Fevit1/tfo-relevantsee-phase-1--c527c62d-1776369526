 ...
 * 1. Validates the Supabase session server-side.
 * 2. Fetches the user's role from the `users` table — NEVER from JWT claims.
 * 3. Returns { user, role, accountId } on success, throws on failure.
 *
...
export async function getAuthenticatedUser(request) {
  ...
  // Fetch role from DB — never trust JWT claims
...
export function requireRole(allowedRoles) {
  return function (handler) {
    return async function (req, context) {
      let authContext
      try {
        authContext = await getAuthenticatedUser(req)
      } catch (err) {
